 ...
 async function ajoutCartable(){
    ...
    async function deleteToCart(product) {
     ...
     try {
       ...
       const response = await fetch(`http://127.0.0.1:8000/panier?article_id=${product}&type_id=2`, {
         method: "DELETE",
         mode: "cors",
         headers: {
           Accept: "application/json",
   ...
   <div class = "container">
    ...
    <div class = "item1">
     <p style="margin-right: 80px;">Total ({nbarticle} articles) : {total}€</p>
        <button class = "button1" on:click={ajoutCartable}>Ajouter au cartable</button>
    </div>
